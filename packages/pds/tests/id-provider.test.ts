...
describe("ID provider creation", () => {
  let network: TestNetworkNoAppView;
  let agent: AtpAgent;

  beforeAll(async () => {
    network = await TestNetworkNoAppView.create({
      dbPostgresSchema: "id_provider_creation",
    });
  ...
  const createIdentityProvider = async (data) => {
    const { success } = await agent.com.atproto.admin.createIdentityProvider(
      data,
      {
